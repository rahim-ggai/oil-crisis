...
const STATUS_LABELS: Record<CargoStatus, string> = {
  docked: 'Docked',
  in_war_zone: 'In War Zone',
  outside_war_zone: 'Outside War Zone',
  contracted_not_dispatched: 'Contracted',
};
  ...
  return (
    ...
    >
      <p className="text-xs text-slate mb-4">
        Seed data is illustrative. Edit vessel details, adjust statuses, and add or remove cargoes as needed.
      </p>
          ...
          <table className="w-full text-xs">
            <thead>
              <tr className="border-b border-border">
                <th className="text-left py-2 pr-2 font-medium text-slate">Vessel</th>
                <th className="text-left py-2 pr-2 font-medium text-slate">IMO</th>
                <th className="text-left py-2 pr-2 font-medium text-slate">Product</th>
                <th className="text-right py-2 pr-2 font-medium text-slate">Qty (bbl)</th>
                <th className="text-right py-2 pr-2 font-medium text-slate">Qty (t)</th>
                <th className="text-left py-2 pr-2 font-medium text-slate">Port</th>
                <th className="text-left py-2 pr-2 font-medium text-slate">ETA</th>
                <th className="text-left py-2 pr-2 font-medium text-slate">Insurer</th>
                <th className="text-left py-2 pr-2 font-medium text-slate">Flag</th>
                <th className="text-left py-2 pr-2 font-medium text-slate">Status</th>
                <th className="text-right py-2 pr-2 font-medium text-slate">Loss P</th>
          ...
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={chartData} margin={{ top: 5, right: 20, left: 10, bottom: 40 }}>
              ...
              <YAxis
                ...
                label={{ value: 'Barrels', angle: -90, position: 'insideLeft', offset: 5, fontSize: 11, fill: '#64748b' }}
              />
              <Tooltip
                contentStyle={{ fontSize: 11, background: '#ffffff', border: '1px solid #e2e0dc' }}
                formatter={(value: unknown) => [Number(value).toLocaleString(), 'Barrels']}
              />
    ...
    <tr className="border-b border-border/50 hover:bg-card-hover/50">
      ...
      <td className={cellClass}>
        <input
          ...
          onChange={(e) => onUpdate(cargo.id, { quantityBarrels: parseFloat(e.target.value) || 0 })}
          step={10000}
          min={0}
        />
      </td>
      <td className={cellClass}>
        <input
          ...
          onChange={(e) => onUpdate(cargo.id, { quantityTonnes: parseFloat(e.target.value) || 0 })}
          step={1000}
          min={0}
        />
      </td>
      ...
      <td className={cellClass}>
        <select
          ...
          style={{ minWidth: 120 }}
          value={cargo.status}
          onChange={(e) => onStatusChange(cargo.id, e.target.value as CargoStatus)}
        >
      ...
      <td className={cellClass}>
        <input
          ...
          onChange={(e) => onUpdate(cargo.id, { lossProbability: parseFloat(e.target.value) || 0 })}
          step={0.01}
          min={0}
          max={1}
        />
