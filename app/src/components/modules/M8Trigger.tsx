...
const WEIGHT_FIELDS: { key: keyof TriggerWeights; label: string; description: string }[] = [
...
const HARD_OVERRIDE_DESCRIPTIONS = [
  { id: 'critical-stock', label: 'Critical stock override', condition: 'HSD or MS days of cover < 7 days', level: 'EMERGENCY' as TriggerLevel },
  { id: 'no-cargoes', label: 'No incoming cargoes', condition: 'Days of cover < 14 and no cargoes arriving within 7 days', level: 'EMERGENCY' as TriggerLevel },
  { id: 'price-reserves', label: 'Price + reserves squeeze', condition: 'Brent > 4x baseline and reserves < $12B', level: 'AUSTERITY' as TriggerLevel },
  { id: 'iran-closed', label: 'Iran corridor closed', condition: 'Iran corridor score = 0 and alternates < 30%', level: 'AUSTERITY' as TriggerLevel },
];

const OVERRIDE_OPTIONS: (TriggerLevel | 'none')[] = ['none', 'NORMAL', 'ALERT', 'AUSTERITY', 'EMERGENCY'];

// ────────────────────────────────────────────────────────────
// Stress Dial SVG
// ────────────────────────────────────────────────────────────
function StressDial({ stress, level }: { stress: number; level: TriggerLevel }) {
  const width = 320;
  const height = 190;
  const cx = width / 2;
  const cy = 165;
    ...
    <div className="flex flex-col items-center">
      <svg width={width} height={height} viewBox={`0 0 ${width} ${height}`}>
        ...
        {[0, 25, 50, 75, 100].map((tick) => {
          ...
          return (
            <g key={tick}>
              <line x1={x1} y1={y1} x2={x2} y2={y2} stroke="#64748b" strokeWidth={1} />
              <text
                x={lx}
                y={ly}
                textAnchor="middle"
      ...
      <div
        ...
        style={{ color: LEVEL_COLORS[level], backgroundColor: `${LEVEL_COLORS[level]}14` }}
      >
        {LEVEL_LABELS[level]}
      </div>
    </div>
      ...
      <ResponsiveContainer width="100%" height="100%">
        <BarChart data={data} layout="vertical" margin={{ top: 5, right: 30, left: 100, bottom: 5 }}>
          ...
          <Tooltip
            ...
            formatter={(value: unknown) => [Number(value).toFixed(2), 'Weighted contribution']}
          />
...
export function M8Trigger() {
  ...
  const handleOverrideChange = useCallback(
    (val: string) => {
      updateM8({ manualOverride: val === 'none' ? null : (val as TriggerLevel) });
    },
    [updateM8],
  );
  ...
  const breakdownData: BreakdownEntry[] = useMemo(() => {
    const { stressD, stressP, stressS, bufferA, bufferI } = triggerOutput.components;
    const w = m8.weights;
    return [
      ...
      { name: 'Alternates buf. (A)', value: -(w.wA * bufferA * 0.5), fill: '#27ae60' },
      { name: 'Iran corridor (I)', value: -(w.wI * bufferI * 0.5), fill: '#2563eb' },
    ];
  ...
  const activeOverrides = useMemo(() => {
    ...
    const hsdDays = m1.hsdDailyConsumption > 0 ? m1.hsdStock / m1.hsdDailyConsumption : Infinity;
    const msDays = m1.msDailyConsumption > 0 ? m1.msStock / m1.msDailyConsumption : Infinity;
    ...
    const next7 = m2.cargoes.filter((c) => {
      const diff = (new Date(c.eta).getTime() - Date.now()) / (1000 * 60 * 60 * 24);
      ...
      <div className="flex gap-6">
        ...
        <div className="w-[280px] shrink-0 space-y-4">
            ...
            <div className="flex items-center justify-between pt-2 border-t border-border mt-2">
              ...
              <span
                className={`font-mono text-xs font-semibold ${
                  Math.abs(weightSum - 1) < 0.02 ? 'text-green-muted' : 'text-red-muted'
                }`}
              >
                {weightSum.toFixed(2)}
              </span>
          ...
          <Card title="Manual Override">
            <p className="text-xs text-slate mb-2">
              Committee can force a specific level regardless of computed stress.
            </p>
            <select
          ...
          <Card title="De-escalation Rule">
            <p className="text-xs text-slate leading-relaxed">
              A level may only be de-escalated after the composite stress remains below
              the lower threshold for <span className="font-mono font-semibold text-navy">3 consecutive days</span>.
            </p>
        ...
        <div className="flex-1 space-y-5">
          ...
          <Card title="Component Contributions (Weighted)">
            <p className="text-xs text-slate mb-2">
              Positive values increase stress; negative values (buffers) decrease it.
            </p>
            ...
            <div className="mt-3 pt-3 border-t border-border">
              <div className="grid grid-cols-5 gap-2 text-center">
                {[
                  { label: 'stress_D', value: triggerOutput.components.stressD },
                  { label: 'stress_P', value: triggerOutput.components.stressP },
                  { label: 'stress_S', value: triggerOutput.components.stressS },
                  { label: 'buffer_A', value: triggerOutput.components.bufferA },
                  { label: 'buffer_I', value: triggerOutput.components.bufferI },
                ].map((c) => (
                  <div key={c.label}>
          ...
          <Card title="Hard Override Conditions">
            <p className="text-xs text-slate mb-3">
              These conditions bypass the composite score and force a minimum escalation level.
            </p>
                ...
                return (
                  <div key={ho.id} className="flex items-start gap-3">
                    <span
                      ...
                      style={{
                        backgroundColor: isActive ? LEVEL_COLORS[ho.level] : 'transparent',
                        borderColor: isActive ? LEVEL_COLORS[ho.level] : '#e2e0dc',
                      }}
                    />
                    <div className="flex-1">
                      <div className="flex items-center gap-2">
                        <span className="text-xs font-medium text-navy">{ho.label}</span>
                        <span
                      ...
                      <p className="text-[10px] text-slate mt-0.5">{ho.condition}</p>
                    </div>
                    <span
